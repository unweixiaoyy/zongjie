...
* bindService(Intent，ServiceConnection)流程？
      * 1，客户端进程构造IServiceConnection这个Binder，它包装了客户端编写的ServiceConnection对象，调用AMS.bindService()把IServiceConnection传到AMS进程。2，生成ServiceRecord，把IServiceConnection添加到ServiceRecord中。2.1：调用AMS.scheduleCreateService()进行Service的创建等。2.2：requestServiceBindingsLocked()调用AT.scheduleBindService()，先回调Service.onBind()并返回Service所在进程的由子类实现Service的onBind()返回的Binder对象，再调用AMS.publishService(IBinder)并携带这个Binder，找到客户端进程的IServiceConnection保存在AMS的代理，然后调用connected(IBinder)，经过Binder驱动，客户端进程的ServiceConnection.connected(IBinder)被回调，最终调用ServiceConnection.onServiceConnected(IBinder)方法，完成绑定，2.3：sendServiceArgsLocked()回调onStartCommond()方法。
      * 简单总结：1，客户端Act构造IServiceConnection这个Binder传给AMS，2，AMS存储IServiceConnection并通知客户端Ser创建目标Service并回调onCreate()，3，Service创建完毕，再调用客户端Ser回调onBind方法(onBind由子类返回一个Binder对象)并返回IBinder，4，客户端Ser最后调用AMS.publishService(IBinder)，把创建的IBinder传给AMS，5，AMS拿着这个IBinder，使用IServiceConnection.connected(IBinder)，经过Binder驱动，最终客户端Act的IServiceConnection.connected()被回调，经过层层调用，ServiceConnection.onServiceConnected(IBinder)被回调了。
