...
* Java中对象的生命周期？
```java创建(created)：分配内存空间，调用构造函数，按子类到父类顺序调用构造函数、代码块、变量等。
使用(InUse) ：被至少一个强引用指向。不可见(Invisible)：超过作用域且无强引用指向，但有可能被静态变量或JNI引用。不可达(unReachable)：GCRoot无强引用指向这个对象
收集(collected)：垃圾回收期发现该对象不可到达，标记待回收。终结(finalized)：如重写finalize方法则执行finalize()方法，执行完等待回收。对象空间再分配(deallocated)：对象空间被重用
```
* 开启线程的三种方式？
```java
...
```java
sleep是Thread类的方法,wait是Object类中定义的方法。和锁相关的方法都定义在Object类中
...
* 线程如何关闭？
```java
1：使用锁机制或定义volatile变量控制run方法中阻塞状态2：使用interrupted配合isInterrupted()方法中断线程，3：stop方法，过时不推荐，会引起数据不正确
```
* java中的同步的方法？
```java
...
```java
总结：
一：AT#performLauncherActivity首先执行Act#attach，初始化Act中的Window（PhoneWindow），并设置WindowManager；然后回调Act#onCreate并执行setContentView（实际由PW#setContentView执行） 。PW#setContentView做了两件事，1：初始化DecorView、mContentParent；2：把setContentView中的布局Id或View添加到mContentParent中。
二：AT#handleResumeActivity首先回调Act#onResume声明周期，然后取出Act#Window#DecorView和Act#WindowManager，调用WindowManager.addView(decor, l)准备把DecorView添加到Window中。
三：其中Act中mWindowManager实际类型是WMI（WindowManagerImpl），WMI内部所有方法均由单例类WMG（WindowManagerGlobal）代理执行，结论是WindowManager.addView(decor, l)等于WMG#addView(decor，l)。WMG#addView(decor，l)只做了一件事情，构造ViewRootImpl，并调用ViewRootImpl#setView方法。
四：ViewRootImpl#setView执行requestLayout方法，最终调用scheduleTraversals方法。scheduleTraversals方法调用Choreographer#postCallback执行TraversalRunnable。在TraversalRunnable中最终调用performTraversals方法来依次执行ViewRootImpl的performMeasure、performLayout、performDraw！
五：然后从DecorView开始，遍历每一个子View，一次完成Measure、Layout、Draw。
六：AT#handleResumeActivity最后调用Act#makeVisible方法，把DecorView设置为VISIBLE！
七：Activity中的某一个View如何更新UI？1：首先在VRImpl#setView方法为DecorView设置ViewParent为VRImpl，而DecorView所有子View被添加到ViewGroup中都把ViewGroup当作自己的ViewParent(ViewGroup实现ViewParent接口)，当DecorView初始化完成后，所有View均设置好其ViewParent；2：View刷新方法requestLayout、invalidate、postInvalidate最后均调用VRImpl#scheduleTraversal方法完成更新UI！


* schedule（丝该丢_） Traversal（戳握搜_）Choreographer（卡rui_哥佛_）
```
